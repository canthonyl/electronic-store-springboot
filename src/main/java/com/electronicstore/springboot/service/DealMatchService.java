...
public class DealMatchService {
    ...
    /*private double applyAndUpdateQtyBasedDiscount(DiscountRule r, DealMatchContext dmc, Map<Long, Map<ThresholdType, Double>> thresholdValues, Set<Long> threshold, Set<Long> applicable) {
        Map<Long, Integer> thresholdUnitDefinition = dmc.ruleGroupProductQuantity.get(r.getRuleGroupId());
        Map<Long, Double> quantities = threshold.stream().collect(toMap(Function.identity(), i->thresholdValues.get(i).get(ThresholdType.Qty)));

        long actualApply = numberOfUnits(r, thresholdUnitDefinition, quantities);
        if (actualApply == 0L) return 0.0;
        double applicableAmount = applicableAmountByQty(actualApply, dmc, applicable, r, thresholdValues);


        return applicableAmount * r.getApplicableDiscount();
    }*/

    /*private double applyAndUpdateQtyBasedDiscount(DiscountRule r, DealMatchContext dmc, Map<Long, Map<ThresholdType, Double>> thresholdValues, Set<Long> product) {
        Double qty = product.stream().map(thresholdValues::get).mapToDouble(m->m.get(ThresholdType.Qty)).sum();
        Double amount = product.stream().map(thresholdValues::get).mapToDouble(m->m.get(ThresholdType.Amount)).sum();
    ...
    /*private Map<Long, Map<ThresholdType, Double>> reduceQuantity(long numberOfUnits, Map<Long, Map<ThresholdType,Double>> thresholdValues, Map<Long, Integer> thresholdUnitDefinition, Set<Long> productIds) {
        Map<Long, Map<ThresholdType, Double>> result = new HashMap<>();
        for (Long id : productIds) {
            Map<ThresholdType,Double> values = thresholdValues.get(id);
            double currentQty = values.get(ThresholdType.Qty);
            ...
            double quantityReduced = numberOfUnits * thresholdUnitDefinition.get(id);
            double amountReduced = currentAmount * (quantityReduced / currentQty);
            values.put(ThresholdType.Qty, currentQty - quantityReduced);
            values.put(ThresholdType.Amount, currentAmount - amountReduced);
    ...
    /*private long numberOfUnits(DiscountRule rule, Map<Long, Integer> thresholdUnitDefinition, Map<Long, Double> quantities) {
        long multiplier = rule.getThresholdUnit();
        long numUnits = Long.MAX_VALUE;
        for (Long productId : quantities.keySet()) {
            long threshold = thresholdUnitDefinition.get(productId) * multiplier;
            long quantity = quantities.get(productId).longValue();
            long ratio = quantity / threshold;
            numUnits = Math.min(ratio * rule.getApplicableUnit(), numUnits);
            if (numUnits == 0L) break;
        }
        return numUnits;
    }*/
